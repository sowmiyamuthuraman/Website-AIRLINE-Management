...
<input name="yes" style="display:inline;" type="text" placeholder="year" >&nbsp;&nbsp;
<input type="submit" name="pup" class="btnSearch" style="display:inline;">&nbsp;&nbsp;
</form>
<form  style="display:inline;" action="year_excel_list.php">
<input type="submit" name="export" class="btnSearch" style="float:right;display:inline" value="EXPORT">
</form>
<?php
       ...
       if(isset($_POST['pup'])){
		...
		$a=$_POST['yes'];
	session_start();
	$_SESSION['y1']=$_POST['yes'];
	
			$result=mysql_query("SELECT * FROM leave_request where  YEAR(leave_from)='$a' AND status='accept'");
			...
			{
				echo "<div style='background-color:#A1C6D9;' align='center'>NO RECORDS FOR THE REQUESTED YEAR</div>";
			}
		}
